...
var (
	...
	GVRKhulnasoftNetworkPolicy = &schema.GroupVersionResource{
		Group:    "khulnasoft.io",
		Resource: "khulnasoftnetworkpolicies",
		Version:  "v2",
	}
	ResourceToKind = map[string]string{
		...
		GVRKhulnasoftNetworkPolicy.Resource:   "KhulnasoftNetworkPolicy",
	}
	CatchAllToleration = v1.Toleration{
		Operator: "Exists",
	}
	ScaffoldLabel         = "khulnasoft.scaffold"
	ScaffoldLabelSelector = fmt.Sprintf("app.kubernetes.io=%s", ScaffoldLabel)
)

// WithScaffoldLabel adds a scaffold specific variable to the given unstructured resource.
// This is useful to be able to watch resources created by the scaffold toolkit.
func WithScaffoldLabel(res *unstructured.Unstructured) *unstructured.Unstructured {
	res.Object["metadata"].(map[string]interface{})["labels"] = map[string]interface{}{
		"app.kubernetes.io": ScaffoldLabel,
	}
	return res
}
