#!/usr/bin/env python3
# This is a modified version of the terraform.py script included
# with kubespray, which enables support for hybrid unbonded
# mode equinix metal deployments.
#
...
def convert_to_v3_structure(attributes, prefix=''):
    ...
    result = {}
    if isinstance(attributes, str) or isinstance(attributes, int):
        # In the case when we receive a string (e.g. values for security_groups)
        # Can also receive an int.
...
def query_list(hosts):
    ...
    for name, attrs, hostgroups in hosts:
        ...
        # This ensures that the node's IP from Kubernetes' perspective
        # is the one part of the L2 network, not the L3 network.
        if "master" in name:
            attrs["ip"] = "192.169.0.100"
        elif "node-1" in name:
            attrs["ip"] = "192.169.0.101"
        else:
            attrs["ip"] = "192.169.0.102"
...
def main():
    ...
    # Ansible doesn't provide arguments to this file, so take out
    # required=True here.
    ...
    if args.list:
        ...
        print(output)
    else:  # If no arguments are presented, then print the inventory.
        output = query_list(hosts)
        print(json.dumps(output, indent=4 if args.pretty else None))
