...
# wait for khulnasoft to be ready and then run a connectivity test
# namespace khulnasoft-test is deleted afterwards on success
# SKIP_CT: set to "skip-ct" to skip the connectivity test
wait_khulnasoft_ready() {
    wait_ready
    $ARTIFACTS/khulnasoft status --wait --wait-duration=1m

    if [ "$SKIP_CT" != "skip-ct" ]
    then
        $ARTIFACTS/khulnasoft connectivity test
        kubectl delete ns khulnasoft-test
    fi
}
