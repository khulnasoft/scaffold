...
# don't do a connectivity test for khulnasoft 
SKIP_CT=""
if [ "${1}" == '-no-ct' ]
then
    SKIP_CT="skip-ct"
...
# get khulnasoft cli for install/status
if ! [ -f "$ARTIFACTS/khulnasoft" ]
then
    cd $ARTIFACTS
    $SCRIPT/get_khulnasoftcli.sh
    cd $SCENARIO_DIR
fi

# -- variables --
# anything in all caps is used in a global scope
KHULNASOFT_VERSION=1.11.6  # target khulnasoft version to install
...
node_prep() {
    ...
    do
        ...
        $SSH $node "sudo /bin/bash -c 'echo '"'"'net.ipv4.conf.*.rp_filter = 0'"'"' > /etc/sysctl.d/99-override_khulnasoft_rp_filter.conf && systemctl restart systemd-sysctl'"
    done
}
...
# see https://docs.khulnasoft.com/en/v1.9/gettingstarted/kubeproxy-free/
delete_kube_proxy() {
    set +e
...
# ensure that khulnasoft is installed with KubeProxyReplacement set to strict
# verifies by using khulnasoft status on a node
# see https://docs.khulnasoft.com/en/v1.12/gettingstarted/kubeproxy-free/#validate-the-setup
ensure_strict_kpr() {
    kubectl -n kube-system exec ds/khulnasoft -- khulnasoft status | grep KubeProxyReplacement | grep Strict
}

# basic install of khulnasoft, no kube-proxy replacement (kpr), dsr, xdp
install_khulnasoft_no_kpr() {
    $ARTIFACTS/khulnasoft install \
        --version=$KHULNASOFT_VERSION
}

# install khulnasoft with kube-proxy replacement (kpr), no dsr or xdp
# deletes kube-proxy if it exists in the cluster
install_khulnasoft_kpr_no_xdp() {
    delete_kube_proxy
    wait_ready
    $ARTIFACTS/khulnasoft install \
        --version=$KHULNASOFT_VERSION \
...
# install khulnasoft with kub-proxy replacement (kpr), dsr and xdp acceleration
# deletes kube-proxy if it exists in the cluster
install_khulnasoft_kpr_xdp() {
    delete_kube_proxy
    wait_ready
    $ARTIFACTS/khulnasoft install \
        --version=$KHULNASOFT_VERSION \
...
# Install khulnasoft if needed
if ! $ARTIFACTS/khulnasoft status --wait --wait-duration="1s"
then
    if [ "$1" == "nkpr" ]
    then
        install_khulnasoft_no_kpr
