...
scaffold's aim is to provide a framework for writing simple scripts to execute performance benchmarks, with a focus on keeping the process quick, flexible and simple.
...
* `./cmapisrv-mock`: a component which mocks the behavior of the Khulnasoft Cluster Mesh API Server for scalability testing purposes.
* `./egw-scale-utils`: components for scale testing Khulnasoft's Egress Gateway feature.
...
* **`exec_with_registry.sh`**: Find a service with the labels `app.kubernetes.io/part-of=scaffold` and `app.kubernetes.io/name=registry`, port-forward it to localhost on port 5000, execute a given command, then kill the port-forward. Useful for a `(crane|docker|podman) push`.
...
* **`get_khulnasoftcli.sh`**: Download khulnasoft-cli to current directory using instructions from the documentation.
...
It's convention that each resource can be pinned to a node using NodeSelectors and `role.scaffold/<role/>=true` labels, which is useful when we want to dedicate a node for a certain resource, such as netperf server. See below for specifics.

### prometheus

Deploys prometheus into the `monitoring` namespace onto any node labeled `role.scaffold/monitoring=true`, accessible using the service named `prometheus`.
...
Deploys grafana onto a node with the `role.scaffold/monitoring=true` label into the `monitoring` namespace, accessible using the service named `grafana`.
...
* Khulnasoft v1.12 Operator Metrics [grafana.com](https://grafana.com/grafana/dashboards/16612-khulnasoft-operator/)
* Khulnasoft v1.12 Agent Metrics [grafana.com](https://grafana.com/grafana/dashboards/16611-khulnasoft-metrics/)
...
Deploys [node-exporter](https://github.com/prometheus/node_exporter) in the `monitoring` namespace onto any node labeled with `role.scaffold/monitored=true`.
...
* **`pod2pod-client`**: Selects nodes with the label `role.scaffold/pod2pod-client=true`.
* **`pod2pod-server`**: Selects nodes with the label `role.scaffold/pod2pod-server=true`.
...
Each sub-directory within the `scenarios` directory is meant to house resources for running any kind of performance test, using the resources within scaffold.
...
* **`ROOT_DIR`:** Absolute path to the root of scaffold
...
* **`wait_khulnasoft_ready()`:** Call `wait_ready`,  wait one minute for Khulnasoft to show ready through `khulnasoft status`, and then run a connectivity test. The connectivity test can be skipped by setting `SKIP_CT` to `skip-ct`.
...
Perform latency and throughput regression testing between multiple versions of Khulnasoft.
Netperf latency and throughput tests are executed on a specific version of Khulnasoft.
An upgrade will then be performed to a new version of Khulnasoft, and tests will be repeated.

Profiles will be taken on nodes. Tests are run pod-to-pod.

### IPSec testing

Performs the same tests as above, with options for enabling encryption in Khulnasoft.
One can specify installing Khulnasoft with IPSec enabled, wireguard enabled, or no encryption enabled.

### IPSec RPS testing

Performs the same tests as above, but uses a single version of Khulnasoft and includes a
build of Khulnasoft that includes an RPS implementation for IPSec.
